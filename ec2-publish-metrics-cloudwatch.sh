...
GetMataData() {
  ...
  INSTANCE_TYPE=$(curl -sS ${METADATA_BASE_URL} | grep instanceType |awk -F'"' '{print $4}')

  if [ -z $INSTANCE_ID ] || [ -z $INSTANCE_TYPE ] || [ -z $REGION ]; then
    echo "Can not get \'instance-id\' or \'instance-type\' from meta-data."
    exit 1
  fi
}
